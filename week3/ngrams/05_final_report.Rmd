...
Edit the `01_download_1grams.sh` file to download the `googlebooks-eng-all-1gram-20120701-1.gz` file and 
the `02_filter_1grams.sh` file to filter the original 1gram file to only lines where the ngram matches a year (output to a file named `year_counts.tsv`).

Then edit the `03_download_totals.sh` file to down the `googlebooks-eng-all-totalcounts-20120701.txt` and  
file and the `04_reformat_totals.sh` file to reformat the total counts file to a valid csv (output to a file named `total_counts.csv`). 

## Load the cleaned data

Load in the `year_counts.tsv` and `total_counts.csv` files. Use the `here()` function around the filename to keep things portable.
Give the columns of `year_counts.tsv` the names `term`, `year`, `volume`, and `book_count`. 
...
```{r load-counts}
year_counts <- read_tsv('year_counts.tsv', col_names = FALSE)
year_counts <- year_counts |>
  rename(term = X1, year = X2, volume = X3, book_count = X4)
head(year_counts)

total_counts <- read_csv('total_counts.csv', col_names = FALSE) |>
...
The total number of line in total_counts.csv is `r nrow(total_counts)`.

# Part B

> Recreate the main part of figure 3a of Michel et al. (2011). To recreate this figure, you will need two files: 
the one you downloaded in part (a) and the “total counts” file, which you can use to convert the raw counts into proportions. 
...
total_year_counts <- left_join(year_counts, total_counts, by="year")
...
filtered_total_year_counts <- total_year_counts |>
  filter(term == "1883" | term == "1910" | term == "1950") |>
  filter(year >= 1850 & year <= 2012)

filtered_total_year_counts |>
  ggplot(aes(x = year, y = proportion * 10000, color = term)) +
  geom_line() +
  ylab("proportion")
...
Yes. Version 2 of the NGram data produce is similar results to those presented in Michel et al. (2011), which are based on version 1 data.
...
Yes, i think the difference between (b) and (d) lead me reevaluate the result because the proportion is dropping as the total volume is increasing a lot even though the real volume of the terms is kinda increasing. So, if we only look at the proportion, we might think that those mentioning terms are decreasing but indeed, it is increasing in raw count.
...
```{r compute-peaks}
max_proportion_by_term <- total_year_counts |>
  group_by(term) |>
  slice_max(order_by = proportion, n = 1, with_ties = FALSE) |>
  ungroup()

filtered_max_proportion_by_term <- max_proportion_by_term |>
   filter(year >= 1850 & year <= 2012)
